...
export function memoryWatchMode(): { mode: "node" | "poll"; intervalMs?: number } {
  const value = process.env.CHOKIDAR_USEPOLLING?.toLowerCase();
  // Keep this plugin-owned policy at the public dependency boundary. fs-safe
  // still verifies trusted procfs before Node/Linux native admission. Explicit
  // false/0/empty requests never silently fall back on unsupported runtimes.
  const polling =
    value === undefined
  ...
  const configured = Number.parseInt(process.env.CHOKIDAR_INTERVAL ?? "", 10);
  return {
    mode: "poll",
    intervalMs:
      Number.isInteger(configured) && configured > 0 && configured <= 2_147_483_647
        ? Math.max(20, configured)
        : 100,
  };
}
