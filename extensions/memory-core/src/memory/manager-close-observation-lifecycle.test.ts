...
describe("MemoryIndexManager observation close lifecycle", () => {
  ...
  afterAll(async () => {
    ...
    // file owns its managers/poisoned slots until this isolated test process ends.
  ...
  it.each([false, true])(
    ...
    async (failProvider) => {
      ...
      const physicalFailure = new Error("observer worker join failed");
      ...
      try {
        ...
        // observation failure or release that observer's uncertain worker slot.
