...
describe("Codex network proxy config admission", () => {
  ...
  it.each([
    { field: "appServer.headers", config: { headers: { "private-header-key": false } } },
    {
      field: "appServer.networkProxy.unixSockets",
      config: { networkProxy: { enabled: true, unixSockets: { "/private/socket": "bad" } } },
    },
    { field: "appServer", config: { "private-unknown-key": "private-value" } },
  ])("reports only schema-owned names for $field", ({ field, config }) => {
    expect(() =>
      resolveRuntimeForTest({
        pluginConfig: { appServer: { networkProxy: { enabled: true }, ...config } },
      }),
    ).toThrow(
      new Error(
        `Invalid plugins.entries.codex.config.${field}; fix this field before starting Codex with network restrictions. Run "openclaw doctor --fix" for supported repairs.`,
