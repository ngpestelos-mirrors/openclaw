...
export async function preflightDiscordMessage(
  ...
  // Addressed requests retain their attachments through queueing. Background
  // observation records text and native reply links without downloading media.
  ...
  if (!isDirectMessage) {
    ...
    for (const { message: source } of hydratedSources) {
      const media: DiscordMediaInfo[] = shouldProcessRequest
        ? await resolveMediaList(source, params.mediaMaxBytes, mediaResolveOptions)
        : [];
      if (params.abortSignal?.aborted || params.isPolicyCurrent?.() === false) {
        return null;
      }
      ...
      conversationHistory = await recordConversationObservation(scope, {
        ...
        message: {
          ...
          media: toHistoryMediaEntries(media, { messageId: sourceId }),
          transport: {
            channel: "discord",
            conversationRef: conversation.conversationRef,
            messageId: sourceId,
