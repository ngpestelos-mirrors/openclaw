...
describe("setup surface helpers", () => {
  ...
  describe("group access policy", () => {
    ...
    it.each(["default", "secondary"])(
      "blocks mentioned chat through the selected %s runtime after Disabled",
      async (accountId) => {
        setTwitchRuntime(createPluginRuntimeMock());
        const initialAccount: TwitchAccountConfig = { ...mockAccount, allowedRoles: ["all"] };
        const cfg = {
          channels: {
            twitch:
              accountId === "default"
        ...
        const disabled = groupAccess.setPolicy({ cfg, accountId, policy: "disabled" });
        const account = getAccountConfig(disabled, accountId);
        if (!account) {
          throw new Error("expected configured account");
        }
        const result = await checkTwitchAccessControl({
          account,
          accountId,
