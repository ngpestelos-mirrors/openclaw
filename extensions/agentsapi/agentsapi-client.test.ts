...
describe("Agents API self-hosted session connection", () => {
    ...
    fetchWithSsrFGuardMock.mockImplementation(async () => ({
      response: Response.json({
        ...
        required_actions: [
          ...
          { type: "function_call", turn_id: "turn-fixture", call_id: "call-fixture", name: "fixture_tool", arguments: {} },
        ],
      }),
  ...
  it.each([
  ...
  ])("rejects unsupported connection actions for $type/$requestedId", async ({ type, requestedId }) => {
    fetchWithSsrFGuardMock.mockResolvedValue({
      response: Response.json({
        id: "session-fixture", status: "requires_action", error: null,
        environment: { type, id: "environment-fixture", workspace_directory: "/fixture/workspace", remote_url: "wss://executor.invalid/session-fixture" },
    ...
    const client = new AgentsApiClient("fixture-not-a-real-api-key", vi.fn());
    await expect(client.pendingFunctionCalls("session-fixture", new AbortController().signal))
