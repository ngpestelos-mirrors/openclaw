...
describe("mock deferred subagent tool surface", () => {
  ...
  it.each([
    "sessions_spawn is unavailable. Do not call it.",
    "Available deferred-schema tools:\n- sessions_spawn: Old catalog.\n\nAvailable deferred-schema tools: none.",
  ])("rejects negative or superseded catalog text: %s", async (instructions) => {
    const response = await expectNonStreamingResponsesJson(server, {
      tools: deferredTools,
      instructions,
  ...
  it("uses the current request catalog instead of an inherited developer catalog", async () => {
    const response = await expectNonStreamingResponsesJson(server, {
      tools: deferredTools,
      instructions: "Available deferred-schema tools: none.",
  ...
  it.each([false, true])(
    "does not invent a deferred capability without both catalog and dispatcher (catalog=%s)",
    async (advertised) => {
      const response = await expectNonStreamingResponsesJson(server, {
        tools: advertised
          ? deferredTools.filter((tool) => tool.name !== "tool_call")
          : deferredTools,
