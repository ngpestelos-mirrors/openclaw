...
describe("serialized Ollama schema compatibility controls", () => {
  it.each([
    {
      label: "implicit nested record",
      parameters: { type: "object", properties: { args: { type: "object" } } },
      expected: { type: "object", properties: { args: { type: "object" } } },
    },
    {
      label: "explicit nested record",
      parameters: {
        type: "object",
    ...
    },
    {
      ...
      expected: {
        type: "object",
        properties: { args: { type: "object", additionalProperties: false, properties: {} } },
      },
    },
    {
      label: "root fallback",
  ...
  ])("retains $label across the native chat request", async ({ parameters, expected }) => {
    const { request } = await runToolCallScenario(
      { name: "schema_control", description: "Synthetic schema compatibility check", parameters },
      {},
    );
    expect(request.tools?.[0]?.function?.parameters).toEqual(expected);
  });
});
