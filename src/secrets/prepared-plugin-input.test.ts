...
describe("prepared plugin secret input authority", () => {
  it("revokes prepared reads at quiescence before admitted work finishes", async () => {
    activate("synthetic-current");
    const instance = createOwnedInstance();
    const release = createDeferredCore();
    const pending = instance.run(async () => {
    ...
    try {
      ...
      instance.resume();
      expect(instance.run(() => getPreparedPluginSecretInput(pluginId, "apiKey")).value).toBe(
        "synthetic-current",
      );
    } finally {
      release.resolve();
  ...
  it("rejects an expired invocation token while its instance remains accepting", async () => {
    activate("synthetic-current");
    const instance = createOwnedInstance();
    const release = createDeferredCore();
    let delayed: Promise<ReturnType<typeof getPreparedPluginSecretInput>> | undefined;
    instance.run(() => {
      // This detached continuation inherits the exact frame, not a live call lease.
      delayed = release.promise.then(() => getPreparedPluginSecretInput(pluginId, "apiKey"));
    });
    try {
      release.resolve();
  ...
  it("does not accept a reusable plugin ID scope without an invocation", () => {
    activate("synthetic-current");
    const result = withPluginRuntimePluginScope(
      { pluginId, pluginSource: "/synthetic/owner.ts", pluginOrigin: "global" },
      () => getPreparedPluginSecretInput(pluginId, "apiKey"),
    );
