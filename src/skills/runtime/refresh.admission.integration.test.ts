...
it.skipIf(!nativeSupported)(
  ...
  async () => {
    ...
    await fs.symlink(linkedRoot, sourceRoot, "dir");
...
it.skipIf(!nativeSupported).each(["directory", "missing"] as const)(
  ...
  async (initialKind) => {
    ...
    const unsubscribe = refresh.registerSkillsChangeListener((event) => {
      ...
      if (event.reason === "watch-available") {
        available.resolve({
          ...
          observedDirectories: targetSubscription?.health().observedDirectories ?? 0,
        });
      }
    ...
    try {
      ...
      await fs.symlink(targetRoot, sourceRoot, "dir");
      release.resolve();
      const lexical = await lexicalStarted.promise;
      await lexical.ready;
      await Promise.resolve();
      expect(lexical.health()).toMatchObject({ state: "ready", mode: "node" });
      ...
      expect(targetSubscription?.health()).toMatchObject({ state: "ready", mode: "node" });
