...
/** Revalidate the mint intent at the same live mutation boundary as its approval. */
export function assertExecApprovalStandingGrantCurrent<TPayload>(
  options: Pick<ExecApprovalManagerOptions<TPayload>, "resolveStandingGrantMint">,
  record: ExecApprovalRecord<TPayload> | undefined,
  expected: OperatorStandingGrantMintSpec | undefined,
): void {
  if (
    expected &&
    record &&
    (JSON.stringify(options.resolveStandingGrantMint?.(record.request)) !==
      ...
      (expected.kind === "mcp-tool" && record.mcpToolApprovalActive?.() !== true))
  ) {
