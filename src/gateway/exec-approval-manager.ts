...
> extends ExecApprovalExpiry<TPayload> {
  ...
  async register(
    ...
    return this.trackMutation(async () => {
      ...
      assertCurrent();
      const persistence = captureExecApprovalMutationPersistence(this.options.persistence);
      ...
      assertCurrent();
      const inserted = await runWithExecApprovalMutationPersistence(persistence, () =>
        insertOperatorApproval({
          approval,
      ...
      const promise = this.registerEntry(record, persistence, inserted.record.resolutionRef);
  ...
  async resolveDetailed(
    ...
    const retainedPersistence = localEntry?.persistence;
  ...
  async forceDenyDetailed(
    ...
    const retainedPersistence = capturedEntry?.persistence;
    const localRecord = capturedEntry?.record;
    // Cancellation closes executable authority before its durable CAS can yield.
    if (!this.retired && status === "cancelled" && localRecord) {
      localRecord.approvalAuthority = () => false;
    }
  ...
  /** Carry physical read provenance with a result; adapters never choose a pending binding's store. */
  async readDurableLookup(
    recordId: string,
    authority?: ExecApprovalReadAuthority,
    allowTransportRef = false,
  ): ReturnType<typeof readExecApprovalOwnedLookup> {
    authority?.assertCurrent();
    const record = this.getLocalSnapshot(recordId, allowTransportRef);
    const persistence =
      (record ? this.pending.get(record.id)?.persistence : undefined) ??
      captureExecApprovalMutationPersistence(this.options.persistence);
    return readExecApprovalOwnedLookup(
      persistence,
      record?.id ?? recordId,
      authority,
      allowTransportRef,
  ...
  /** Observe terminal publication only through the pending binding's physical owner. */
  async reconcileDurableTerminal(record: OperatorApprovalRecord): Promise<boolean> {
    const current = await this.reconcileDurableLookup({ outcome: "found", record });
    return current !== null && this.wasTerminalPublished(current);
  }
  ...
  async reconcileDurableLookup(
    ...
    authority?.assertCurrent();
    if (waited || (entry && lookup.persistence !== entry.persistence)) {
      const refreshed = await this.readDurableLookup(recordId, authority);
      authority?.assertCurrent();
      lookup =
    ...
    if (this.retired || this.pending.get(recordId) !== entry) {
      return null;
    }
    this.assertPendingPersistenceCurrent(entry);
  ...
  private async persistStorageCorruptDeny(
    ...
    const result = await runWithExecApprovalMutationPersistence(localEntry.persistence, () =>
      forceDenyOperatorApproval({
        id: recordId,
        status: "denied",
        ...
        expectedKind: this.approvalKind,
        runtimeEpoch: localEntry.persistence.runtimeEpoch,
        databaseOptions: localEntry.persistence.databaseOptions,
  ...
  async consumeAllowOnce(recordId: string, consumerId = recordId): Promise<boolean> {
    ...
    return this.trackMutation(async () => {
      ...
      const result = await runWithExecApprovalMutationPersistence(persistence, () =>
        consumeOperatorApprovalAllowOnce({
          id: recordId,
          nowMs,
          consumerId,
