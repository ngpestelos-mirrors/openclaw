...
export async function loadVisibleApproval(params: {
  ...
  ].find((manager) => manager?.getLocalSnapshot(params.id, params.allowTransportRef));
  const liveRecord = localManager?.getLocalSnapshot(params.id, params.allowTransportRef);
  if (
    liveRecord &&
  ...
  let bindingId = liveRecord?.id ?? params.id;
  let admittedRecord = liveRecord?.resolvedAtMs === undefined ? liveRecord : undefined;
  let admittedManager = admittedRecord ? localManager : undefined;
  ...
  try {
    lookup = localManager
      ? await localManager.readDurableLookup(params.id, lookupAuthority, params.allowTransportRef)
    ...
    const corrupt = { outcome: "corrupt", id: bindingId } as const;
  ...
  const missing = {
    ...
    id: lookup.outcome === "corrupt" ? (lookup.id ?? bindingId) : bindingId,
  } as const;
