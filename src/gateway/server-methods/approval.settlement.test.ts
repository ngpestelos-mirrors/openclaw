...
it.each([
  { resolverId: null, rollback: false },
  { resolverId: "shared-runtime", rollback: true },
])(
  "closes an unattributable committed runtime verdict at its deadline ($resolverId; rollback=$rollback)",
  async ({ resolverId, rollback }) => {
    const state = expectDefined(sharedState, "shared approval test state");
    const databaseOptions = { env: state.env };
    const manager = new ExecApprovalManager({
      persistence: { runtimeEpoch: "unattributed-deadline", databaseOptions },
    });
    vi.spyOn(Date, "now").mockReturnValue(Date.now());
    const timers = vi.spyOn(globalThis, "setTimeout");
    const record = manager.create({ command: "echo no invented authority" }, 600_000);
    const { decision } = await manager.register(record, 600_000);
    const otherRecord = manager.create({ command: "echo independent waiter" }, 900_000);
    const other = await manager.register(otherRecord, 900_000);
    const otherResult = vi.fn();
    void other.decision.then(otherResult, otherResult);
    const decisionResult = vi.fn();
    void decision.then(
      (value) => decisionResult({ value }),
      (error: unknown) => decisionResult({ error }),
    );
    const handoff = vi.fn(async () => {});
    const observation = manager.registerDecisionHandoff(record.id, handoff).observation;
    const observed = vi.fn();
    void observation.then(
      () => observed("executed"),
      (error: unknown) => observed(error),
    );
    const timerIndex = timers.mock.calls.findIndex(([, delay]) => delay === 600_000);
    const timer = expectDefined(timers.mock.calls[timerIndex], "registered approval deadline");
    const callback = timer[0];
    if (typeof callback !== "function") {
      throw new Error("Expected approval deadline callback");
    }
    // Invoke the registered deadline deterministically, not a lookup that hides a dead timer.
    clearTimeout(timers.mock.results[timerIndex]?.value);
    resultDelivery.loseResult = "worker";
    resultDelivery.hideReceipt = true;
    try {
      await expect(manager.resolveAutoReview(record.id, resolverId)).rejects.toThrow(
        "result delivery loss",
      );
      const winner = getOperatorApproval({ id: record.id, databaseOptions });
      expect(winner).toMatchObject({
        status: "allowed",
        decision: "allow-once",
        resolver: { kind: "runtime", id: resolverId },
      });
      expect(decisionResult).not.toHaveBeenCalled();
      resultDelivery.hideReceipt = false;
      let expiryFinished = Promise.withResolvers<void>();
      const expire = manager.forceDenyDetailed.bind(manager);
      vi.spyOn(manager, "forceDenyDetailed").mockImplementation((...args) => {
        const result = expire(...args);
        // Keep the exact result promise: expiry resumes before the checkpoint observer.
        void result.then(
          () => expiryFinished.resolve(),
          () => expiryFinished.resolve(),
        );
        return result;
      });
      vi.mocked(Date.now).mockReturnValue(record.expiresAtMs - (rollback ? 5000 : 0));
      installTestApprovalClock();
      callback();
      await expiryFinished.promise;
      if (rollback) {
        expect(decisionResult).not.toHaveBeenCalled();
        const retryIndex = timers.mock.calls.findIndex(([, delay]) => delay === 5000);
        expect(retryIndex).toBeGreaterThanOrEqual(0);
        const retry = timers.mock.calls[retryIndex]?.[0];
        if (typeof retry !== "function") {
          throw new Error("Expected retained original deadline");
        }
        clearTimeout(timers.mock.results[retryIndex]?.value);
        expiryFinished = Promise.withResolvers<void>();
        vi.mocked(Date.now).mockReturnValue(record.expiresAtMs);
        retry();
        await expiryFinished.promise;
      }
      expect(decisionResult).toHaveBeenCalledExactlyOnceWith({ error: expect.any(Error) });
      expect(observed).toHaveBeenCalledExactlyOnceWith(expect.any(Error));
      expect(handoff).not.toHaveBeenCalled();
      expect(otherResult).not.toHaveBeenCalled();
      expect(manager.getLiveSnapshot(otherRecord.id)).toBe(otherRecord);
      expect(manager.consumeAskFallback(record.id)).toBe(false);
      expect(manager.projectDecisionIfActive(record.id, "allow-once")).toBeNull();
      expect(await manager.consumeAllowOnce(record.id)).toBe(false);
      expect(getOperatorApproval({ id: record.id, databaseOptions })).toEqual(winner);
    } finally {
      await manager.drain();
    }
  },
