...
/** Read provenance is carried from the captured physical owner, not inferred from matching row bytes. */
export async function readExecApprovalOwnedLookup(
  persistence: ExecApprovalMutationPersistence,
  recordId: string,
  authority?: ExecApprovalReadAuthority,
  allowTransportRef = false,
): Promise<GetOperatorApprovalResult & { persistence: ExecApprovalMutationPersistence }> {
  authority?.assertCurrent();
  const lookup = await runWithExecApprovalMutationPersistence(persistence, () =>
    getOperatorApprovalDetailed({
      id: recordId,
      nowMs: Date.now(),
      allowTransportRef,
