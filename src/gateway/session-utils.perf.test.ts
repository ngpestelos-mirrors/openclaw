...
import * as sessionList from "./session-utils-list.js";
import {
...
function writeResidentEntries(store: Record<string, SessionEntry>, updatedAtDelta = 0) {
  ...
  test.each([
    ...
    async ({ phase, rosterSize }) => {
      await withStateDirEnv("openclaw-roster-chunks-", async () => {
        ...
        const yieldWork = sessionList.yieldSessionListWork;
        const yields = vi
          .spyOn(sessionList, "yieldSessionListWork")
    ...
    await withStateDirEnv("openclaw-perf-acp-", async () => {
      ...
      try {
        ...
        try {
          ...
          expect(
            projection.snapshot({ agentId: "default", key: missingKey }).row
