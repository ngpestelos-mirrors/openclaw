...
export abstract class ExecApprovalLifecycle<TPayload> {
  ...
  protected assertPendingPersistenceCurrent(entry: PendingEntry<TPayload> | undefined): void {
    if (entry) {
      assertExecApprovalMutationPersistenceCurrent(entry.persistence);
    }
  }
  ...
  protected settleLocalFromStore(
    ...
    const entry = this.pending.get(record.id);
    const resolutionSource = resolveExecApprovalResolutionSource(
      record,
      localResolutionSource,
      entry?.uncertainVerdict?.autoReview,
    );
    if (resolutionSource === undefined) {
      // Null/timeout would authorize fallback; reject only this unattributable observation.
      if (entry && entry.record.expiresAtMs <= Date.now()) {
        this.assertPendingPersistenceCurrent(entry);
        ...
        entry.reject(new Error("Approval verdict cannot be attributed to its original resolver"));
      }
      return false;
    }
    const settlement = prepareExecApprovalSettlement({
      ...
      localDecision,
      localResolvedBy,
      localResolutionSource: resolutionSource,
    });
    if (!settlement) {
      return false;
    }
    ...
    if (settled) {
      ...
      if (record.status === "expired" && entry) {
        try {
          this.options.onExpired?.(record, entry.record);
        } catch (error) {
  ...
  protected registerEntry(
    ...
    // Observers may attach after registration; rejection must remain observable, not unhandled.
  ...
  getLocalSnapshot(
    ...
    const entry =
      this.pending.get(recordId) ??
      (allowTransportRef
        ? Array.from(this.pending.values()).find((value) => value.resolutionRef === recordId)
        : undefined);
    if (!entry) {
      return null;
    }
