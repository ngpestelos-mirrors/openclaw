...
/** Runtime resolver IDs are not proof that a particular auto-review operation committed. */
export function resolveExecApprovalResolutionSource(
  record: OperatorApprovalRecord,
  confirmed: ExecApprovalResolutionSource | undefined,
  autoReview: { committedResolutionKey?: string } | undefined,
): ExecApprovalResolutionSource | undefined {
  if (confirmed) {
    return confirmed;
  }
  if (autoReview && record.status === "allowed" && record.decision === "allow-once") {
    if (autoReview.committedResolutionKey === getOperatorApprovalResolutionKey(record)) {
      return "auto-review";
    }
    if (record.resolver?.kind === "runtime") {
      return undefined;
    }
  }
  return "operator";
