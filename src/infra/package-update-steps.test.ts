    ...
    async (cleanupMode) => {
      await withTestDir({ prefix: "openclaw-package-update-staged-cleanup-" }, async (base) => {
        ...
        try {
          ...
          expect(removalAttempts).toBe(process.platform === "win32" ? 12 : 3);
  ...
  it
    ...
    await withTestDir({ prefix: "openclaw-package-update-shim-rollback-" }, async (base) => {
      ...
      const chmodSpy = vi
        ...
        .mockImplementation(async function (this: FileHandle, mode) {
          rejectRestoredMode(mode);
          await realHandleChmod.call(this, mode);
        });
      const copySpy = vi
        .spyOn(prototype, "copyIn")
        .mockImplementation(async function (this: Root, target, source, options) {
