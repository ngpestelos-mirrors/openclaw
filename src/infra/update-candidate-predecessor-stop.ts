...
// v2026.9.6 persists identity in detail. Retain this reader while that published
// updater remains in the supported compatibility inventory; new writes use the retained key.
function decodeLegacyReceipt(detail: string | undefined): StoppedServiceIdentity | undefined {
  if (!detail) {
    return undefined;
  }
  let value: unknown;
  ...
  return {
    ...
    ...(typeof value.managerUid === "number" ? { managerUid: value.managerUid } : {}),
...
function readDoctorStop(runId: string, ledger: UpdateRunLedgerOptions) {
  let legacy: StoppedServiceIdentity | undefined;
  for (const step of getUpdateRun(runId, ledger)?.steps ?? []) {
    ...
    if (!legacy && step.step === "managed-service:candidate-stop") {
      legacy = decodeLegacyReceipt(step.detail);
    }
  }
  return legacy;
}
