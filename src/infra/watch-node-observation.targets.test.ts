...
describe("developer linked-source admission", () => {
  ...
  it("bounds cumulative admitted Roots without manufacturing an errno", async () => {
    const cwd = temp.make("source-repo-");
    await fs.mkdir(path.join(cwd, "src"));
    const alias = path.join(cwd, "src", "linked");
    const discovery = createSourceTargetDiscovery(cwd, ["src"], ignored);
    for (let index = 1; index < SOURCE_OBSERVATION_LIMITS.roots; index++) {
      const outside = temp.make("source-target-");
      await directoryLink(path.join(outside, "missing"), alias);
      await discovery.discover(signal());
      await fs.unlink(alias);
    }
    const outside = temp.make("source-target-");
    await directoryLink(path.join(outside, "missing"), alias);
    await expect(discovery.discover(signal())).rejects.toMatchObject({
      name: "RangeError",
      message: expect.stringContaining("4 lifetime-pinned Root budget"),
    });
  });
