...
function managedRepairSpawnPreload(root: string): string {
  ...
  const recordedWorkers = [
    ...[
      runtimeProcessEntrypoints.serviceChildRelay,
      runtimeProcessEntrypoints.serviceChildGroupAnchor,
    ].map((entry) => path.resolve("dist", entry.distWorkerPath)),
    path.join(root, "candidate", "dist", runtimeProcessEntrypoints.updateRepair.distWorkerPath),
  ];
  return `const fs = require("node:fs");
    ...
    childProcess.spawn = function(command, args, options) {
      // Isolation strips NODE_OPTIONS; pass the recorder only to this fixture's owned workers.
      if (command === process.execPath && Array.isArray(args) && args.length === 1 &&
          ${JSON.stringify(recordedWorkers)}.includes(args[0])) {
