...
async function scan(rows: Row[], previous?: UsageCostRollupEntry, afterFirstPage?: () => void) {
  let onFirstPage = afterFirstPage;
  ...
  return scanUsageCostRollupInWorker({
    ...
    previous: previous ? { entry: previous } : undefined,
    ...
    readRows: async (_marker, afterSeq, throughSeq) => {
      const page = rows.filter((row) => row.seq > afterSeq && row.seq <= throughSeq).slice(0, 2);
      if (afterSeq === 0) {
        onFirstPage?.();
...
describe("paged SQLite usage rollups", () => {
  ...
  it.each(["replace", "truncate"])("rejects checkpoint %s between passes", async (mutation) => {
    ...
    await expect(
      scan(rows, undefined, () => {
        if (mutation === "truncate") {
          ...
          rows[1] = { seq: 4, event: message("changed", "root", 99) };
        }
      }),
    ).rejects.toThrow("SQLite transcript changed while scanning");
  });

  it("keeps the captured checkpoint when a later append races the scan", async () => {
    const rows = structuredClone(initial);
    const result = await scan(rows, undefined, () => {
      rows.push({ seq: 9, event: message("later", "a", 100) });
    });
    expectUsage(result, 3, 2, "a");
    expect(result.checkpoint).toMatchObject({ maxSeq: 4, eventCount: 2 });
  });
});
