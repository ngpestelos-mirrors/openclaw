...
export async function linkUpdateCandidatePluginTrees(
  ...
  const activities = activityScopes.toSorted(deepestFirst);
  const browsers = browserScopes.toSorted(deepestFirst);
  for (const entry of plan.entries) {
    if (entry.kind !== "file") {
      continue;
    }
    const activity = activities.find(([root]) => isPathInside(root, entry.path))?.[1];
    const browser = browsers.find(([root]) => isPathInside(root, entry.path))?.[1];
    if (
      (activity &&
