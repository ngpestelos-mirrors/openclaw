...
describe("isSafeToCopyOAuthIdentity (unified copy gate, used for mirror and adopt)", () => {
  it("loads credential scope only from shipped policy, without ambient plugin selection", () => {
    const load = vi.spyOn(facadeLoader, "loadBundledPluginPublicSurfaceModuleSyncCore");
    try {
      expect(
        isSafeToCopyOAuthRoutingScope(
          { provider: "github-copilot", enterpriseUrl: "acme.ghe.com" },
          { provider: "github-copilot", enterpriseUrl: "other.ghe.com" },
        ),
      ).toBe(false);
