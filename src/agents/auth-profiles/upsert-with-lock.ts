...
import { captureAuthProfileStoreMutation } from "./store-mutation-admission.js";
import {
...
function loadAuthProfileWriteTarget(params: {
  ...
  if (params.agentDir || (!params.stateDir && !params.env)) {
    return loadPersistedAuthProfileStore(params.agentDir);
  }
  if (params.env) {
    return loadPersistedSharedAuthProfileStore(params.env);
  }
...
function loadAuthProfileWriteAuthority(
  params: { agentDir?: string; stateDir?: string; env?: NodeJS.ProcessEnv },
  profileId: string,
...
export async function persistAuthProfileBatch(
  ...
  const captured = captureAuthProfileStoreMutation(params);
  try {
    const observedProfiles = new Map(
      [...profiles.keys()].map((profileId) => [
        profileId,
        loadAuthProfileWriteAuthority(captured, profileId),
      ]),
    );
    ...
    return await withOAuthProfileLocks(
      [...profiles.entries()].flatMap(([profileId, entry]) =>
        entry.credential.type === "oauth"
          ? [{ profileId, provider: entry.credential.provider }]
          : [],
      ),
      () =>
        runAuthProfileMutationAdmission(captured.env, async () => {
          captured.source.admit();
          ...
          const preparedOwner = runAuthProfileWriteTransaction(
            ...
            (database, owner) => {
              ...
              captured.source.admit();
              storeWasAbsent =
                inspectPersistedAuthProfileStoreRaw(captured.agentDir, database).status ===
                "missing";
              stateWasAbsent =
                inspectPersistedAuthProfileStateRaw(captured.agentDir, database).status ===
                "missing";
              const next =
                loadPersistedAuthProfileStore(captured.agentDir, { database }) ??
              ...
              for (const [profileId, entry] of profiles) {
                ...
                if (
                  supersedesOAuthRefreshGenerationObservedAtAdmission({
                    ...
                    allowOAuthGenerationReplacement:
                      params.allowOAuthGenerationReplacement === true,
                  }) ||
            ...
            { sharedStoreWrite: true, env: captured.env },
          );
            ...
            rollback: () => {
              ...
              runAuthProfileWriteTransaction(
                ...
                (database, owner) => {
                  ...
                  const current = loadPersistedAuthProfileStore(captured.agentDir, { database });
                  if (!current) {
                    return;
                  }
                  ...
                  if (storeWasAbsent && Object.keys(current.profiles).length === 0) {
                    deletePersistedAuthProfileStoreRaw(captured.agentDir, database);
                  }
                  if (stateWasAbsent && buildPersistedAuthProfileState(current) === null) {
                    writePersistedAuthProfileStateRaw(null, captured.agentDir, database);
                  }
                },
