...
it.skipIf(process.platform !== "linux")(
  ...
  async () => {
    ...
    const loader = execUrl.pathname.endsWith(".ts")
      ...
      : undefined;
    const brokerUrl = resolveRuntimeWorkerUrl(updateExecutorNativeEntrypoints.processBrokerHost);
    const brokerContextUrl = resolveRuntimeWorkerUrl(
      updateExecutorNativeEntrypoints.processBrokerContext,
    );
...
const { createSpawnBrokerHost } = await import(${JSON.stringify(brokerUrl.href)});
const { runWithSpawnBroker } = await import(${JSON.stringify(brokerContextUrl.href)});
...
try {
  await broker.ready();
  for (const mode of ["runner", "scope", "broker-runner", "broker-scope"]) {
    const runner = mode.endsWith("runner");
    const brokered = mode.startsWith("broker-");
    ...
    const root = \`
      ...
      require("node:fs").writeFileSync(\${JSON.stringify(receipt)},
        JSON.stringify({ root: process.pid, parent: process.ppid, descendant: child.pid }));
      child.once("message", () => {
        require("node:fs").writeFileSync(\${JSON.stringify(receipt)},
          JSON.stringify({ root: process.pid, parent: process.ppid, descendant: child.pid, ready: true }));
        child.disconnect();
        child.unref();
      });
    \`;
    ...
    try {
      await (brokered ? runWithSpawnBroker(broker, execute) : execute());
    } catch (error) {
    ...
    try {
      ...
      assert.equal(pids.parent, brokered ? broker.pid : process.pid,
        "the selected native or broker transport must own the actual root");
      reports.push({
        mode,
        rootCode: runner ? result?.code : result?.exitCode,
        rootSignal: result?.signal ?? null,
        cleanup: runner ? result?.cleanup : failure ? "failed" : "settled",
