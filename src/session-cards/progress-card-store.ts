...
} from "../state/openclaw-agent-progress-card-schema.js";
...
function progressCardTablePresent(db: DatabaseSync): boolean {
  ...
  if (!facts) {
    throw new Error("Progress-card storage requires admitted SQLite schema facts");
  }
  return facts.tables.has(SESSION_PROGRESS_CARDS_TABLE);
}
...
export function writeSessionProgressCard(
  ...
  if (!db.isTransaction) {
    throw new Error("Progress-card mutation requires an active transaction");
  }
  if (!progressCardTablePresent(db)) {
