/**
 * Config that decides what agents and Gateway clients may do: tool and exec
 * policy, sandboxing, approvals, who counts as an owner, and Gateway
 * authorization. A delegated change here always waits for a human decision,
 * even in Full Access, so a run cannot widen its own authority without the
 * owner seeing the exact change.
 */
...
const PERMISSION_POLICY_PATHS: readonly (readonly string[])[] = [
  ...
  // Gateway authorization: credentials and identity mapping, operator roles and
  // scopes, HTTP tool allow/deny, trusted proxies, and node command policy.
  ["gateway", "auth"],
  ["gateway", "roles"],
  ["gateway", "tools"],
