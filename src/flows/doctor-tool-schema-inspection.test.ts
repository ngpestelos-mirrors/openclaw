...
it("inspects each agent through one registration and reports a failed owner without retrying it", async () => {
  await withOpenClawTestState({ env: { OPENCLAW_DISABLE_BUNDLED_PLUGINS: "1" } }, async (state) => {
...
`,
      ...
      agents: {
        ...
        entries: {
          alpha: {
            ...
            tools: { allow: ["fleet_tool", "failed_tool", "requester__*"] },
          },
          beta: {
    ...
    const findings = await withPluginRuntimeRegistryScope(caller.registry, () =>
      check!.detect({
        mode: "doctor",
        cfg,
    ...
    expect(findings.filter((finding) => finding.target === "fleet_tool")).toEqual([
      expect.objectContaining({
        message: expect.stringContaining("Agent alpha tool fleet_tool from plugin fleet-tool"),
