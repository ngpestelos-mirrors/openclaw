...
// Native subscriptions each own a worker; replacement waits for all prior joins.
const MAX_CONFIG_WATCH_WORKERS = 4;
...
function resolveUsePolling(degradedToPolling: boolean): boolean {
  ...
  return defaultFsObservationUsePolling() || Boolean(process.env.VITEST) || degradedToPolling;
}

function configPollingInterval(): number {
...
export function createConfigFileAdapter(opts: {
  ...
  const createWatcher = (replacement: boolean) => {
    ...
    const next: Generation = {
      lifetime,
      mode: resolveUsePolling(degradedToPolling) ? "poll" : "node",
      ready: false,
      subscriptions: [],
    ...
    next.starting = Promise.resolve().then(async () => {
      ...
      for (const boundary of admitted) {
        ...
        for (let offset = 0; offset < allEntries.length; offset += 128) {
          if (next.mode === "node" && next.subscriptions.length >= MAX_CONFIG_WATCH_WORKERS) {
            throw new RangeError(
              "Config observation worker limit reached (4); reduce include scopes or explicitly select polling",
            );
          }
          ...
          const subscription = watch(boundary.authority, {
            scopes,
            mode: next.mode,
            intervalMs: next.mode === "poll" ? configPollingInterval() : undefined,
            ...
            onHealth: (health) => {
              if (health.state === "unavailable" && isCurrent()) {
                handleWatcherError(
                  next,
                  health.error ?? new Error("Config observation unavailable"),
                );
              }
            },
          });
      ...
      if ([...desiredPaths].some((path) => !covered.has(path))) {
        throw new Error("Config watch paths require fresh source-root admission");
      }
  ...
  const handleWatcherError = (source: Generation, error: unknown) => {
    ...
    if (backoff === undefined) {
      if (source.mode === "node" && resolveUsePolling(true)) {
