...
it("commits worker membership and participant facts before publishing, and rejects stale authority", async () => {
  await withOpenClawTestState({ scenario: "minimal" }, async () => {
    ...
    try {
      ...
      expect(changes.at(-1)).toMatchObject({ sessionKey: scope.sessionKey, membership: ["guest"] });
      expect(
      ...
      expect(changes.at(-1)).toMatchObject({ sessionKey: scope.sessionKey, membership: [] });
      expect(
