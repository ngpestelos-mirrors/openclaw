...
export function readSessionMembershipFactsInDatabase(
  ...
  if (sessionKeys?.length === 0) {
    return { kind: "session-membership-facts", identity, facts: [] };
  }
  return withSqlitePostCommitPublications(database.db, () =>
    runSqliteDeferredTransactionSync(database.db, () => {
      ...
      for (const [sessionKey, values] of memberships) {
        ...
        facts.set(sessionKey, [sessionKey, current?.[1] ?? null, values, current?.[3] ?? {}]);
      }
      ...
      return { kind: "session-membership-facts" as const, identity, facts: [...facts.values()] };
    }),
  );
}
