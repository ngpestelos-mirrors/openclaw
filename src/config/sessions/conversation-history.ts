...
export function prepareConversationHistoryInput(
  database: HistoryDatabase,
  capture: ConversationHistoryCapture,
  message: PersistedUserTurnMessage,
): { message: PersistedUserTurnMessage; selection: ConversationHistorySelection } {
  ...
  for (const row of rows) {
    ...
    } else if (!capture.includeMessage || capture.includeMessage(input)) {
      selectedSequences.push(row.seq);
  ...
  const historyText = background
    .map((input) => {
      ...
      return `${native.length ? `[${native.join("; ")}] ` : ""}${sender ? `${sender}: ` : ""}${input.text ?? ""}`;
    })
    .join("\n");
  if (!historyText) {
