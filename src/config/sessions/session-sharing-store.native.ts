...
function publishCommittedSessionMembership(
  ...
  // A later savepoint may revoke an earlier grant before the outer commit.
  deferOpenClawAgentPostCommitPublication(database, () =>
    sessionChanges.emit({
      agentId,
      storePath: database.path,
      sessionKey,
      membership: listSessionMembersInDatabase(database, sessionKey).map(
...
export function addSessionMember(
  ...
  const inserted = runOpenClawAgentWriteTransaction((database) => {
    ...
    if (changed) {
      publishCommittedSessionMembership(database, agentId, sessionKey);
    }
    return changed;
  }, options);
...
export function removeSessionMember(
  ...
  return runOpenClawAgentWriteTransaction((database) => {
    assertAuthorizedSessionInstance(database, sessionKey, expectedSessionId, expectedEntry);
    const db = getSessionMemberKysely(database);
