...
type SessionTranscriptArchiveDatabase = Pick<
...
export function hasPendingSessionTranscriptArchives(
  ...
  return (
    ...
    executeSqliteQueryTakeFirstSync(
      database.db,
      getNodeSqliteKysely<SessionTranscriptArchiveDatabase>(database.db)
...
export function prepareSessionTranscriptArchivePublishPlans(
  ...
  const db = getNodeSqliteKysely<SessionTranscriptArchiveDatabase>(database.db);
