...
import {
  publishLaunchAgentRegistration,
  readLaunchAgentArtifactState,
  type LaunchAgentArtifactState,
} from "./launchd-registration.js";
...
export function resolvePreCanonicalLaunchAgentPlistPath(
...
type LaunchAgentFileSnapshot = { contents: Buffer; mode: number; registrationTarget?: string };
...
type LaunchAgentFileState = LaunchAgentArtifactState;
...
async function captureLaunchAgentFiles(
  paths: string[],
  registration?: { file: string; target: string },
) {
  const readState = (file: string, publicationTarget = file) =>
    readLaunchAgentArtifactState(
      file,
      publicationTarget === registration?.file ? registration.target : undefined,
    );
  ...
  await withGatewayServiceInstallationRecovery(
    async () => {
      for (const file of paths) {
        ...
        const snapshot = state?.registrationTarget
          ? {
              contents: Buffer.from(state.registrationTarget),
              mode: state.mode,
              registrationTarget: state.registrationTarget,
            }
          : await readExistingLaunchAgentPlist(file);
        const contents = snapshot?.contents ?? null;
        if (
          !isDeepStrictEqual(state, await readState(file)) ||
  ...
  const published = new Map<string, LaunchAgentFileState | null>();
  const prepared = new Map<string, LaunchAgentFileState | null>();
  const matchesPublication = (
    current: LaunchAgentFileState | null,
    expected: LaunchAgentFileState | null,
  ): boolean =>
    current === null
      ? expected === null
      : expected !== null &&
        (["dev", "ino", "sha256", "mode", "size", "mtimeMs", "registrationTarget"] as const).every(
  ...
  const verify = async (file: string) => {
    ...
    if (
      published.has(file)
        ? !matchesPublication(current, expected ?? null)
  ...
  const hooks: GatewayServiceDefinitionTransactionHooks = {
    ...
    filePrepared: async (file, temporary) => {
      ...
      const pending = temporary === null ? null : await readState(temporary, file);
      assertGatewayServiceUpdateCurrent();
      if (temporary !== null && !pending) {
    ...
    fileWritten: async (file, contents) => {
      ...
      if (
        !prepared.has(file) ||
        !matchesPublication(current, pending ?? null) ||
        (contents === null
          ? current !== null
  ...
  return {
    ...
    publishRegistration: async () => {
      ...
      await beforeWrite();
      if ((await verify(registration.file))?.registrationTarget === registration.target) {
        return;
      }
      await publishLaunchAgentRegistration({ ...registration, hooks });
    },
    restore: async (): Promise<boolean> => {
      ...
      for (const file of [...published.keys()].toSorted((a, b) => order(a) - order(b))) {
        ...
        if (original.snapshot !== null && original.state) {
          if (original.snapshot.registrationTarget) {
            await publishLaunchAgentRegistration({
              file,
              target: original.snapshot.registrationTarget,
              hooks,
            });
          } else {
            await publishServiceFile({
          ...
          original.state = published.get(file) ?? null;
        } else {
          if ((await verify(file)) !== null) {
            assertGatewayServiceUpdateCurrent();
...
export function captureLaunchAgentInstallFiles(env: GatewayServiceEnv) {
  const label = resolveLaunchAgentLabel(env);
  const target = resolveLaunchAgentPlistPath(env);
  const file = resolvePreCanonicalLaunchAgentPlistPath(env, label);
  return captureLaunchAgentFiles(
    ...
    file === target ? undefined : { file, target },
  );
}
