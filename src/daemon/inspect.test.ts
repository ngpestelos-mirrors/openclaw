...
<key>ProgramArguments</key><array><string>/usr/local/bin/openclaw</string><string>gateway</string><string>--port</string><string>18888</string></array>
  ...
  it("scans boot-volume and raw external LaunchAgents directories when HOME is external", async () => {
    ...
    const canonicalDirectory = path.resolve("/Users/test", "Library", "LaunchAgents");
    const formerDirectory = path.resolve(fsState.externalHome, "Library", "LaunchAgents");
    const canonicalPlist = path.join(canonicalDirectory, "com.example.openclaw-gateway.plist");
    const rawPlist = path.join(formerDirectory, "com.clawdbot.gateway.plist");
