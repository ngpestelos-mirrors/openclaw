...
import { readDaemonRuntimePin, readDaemonRuntimePinForInstall } from "./runtime-pin-state.js";
import type { GatewayServiceCommandConfig, GatewayServiceInstallArgs } from "./service-types.js";
import { resolveGatewayService } from "./service.js";
const formerDefinition = vi.hoisted(() => vi.fn());
vi.mock("./launchd-install.js", async (original) => ({
  ...(await original<typeof import("./launchd-install.js")>()),
  readRelocatedLaunchAgentForInstall: formerDefinition,
}));
...
beforeEach(() => {
  ...
  formerDefinition.mockReset().mockResolvedValue(null);
  native.command = null;
...
it.each([false, true])(
  "revalidates former LaunchAgent runtime intent under the install lock (changed=%s)",
  async (changed) => {
    await withOpenClawTestState({ label: "pin-native-relocation" }, async (state) => {
      const service = resolveGatewayService();
      const scope = { kind: "gateway" as const, env: state.env };
      const pin = { runtime: "node" as const, path: "/runtime/node" };
      const args = {
        env: state.env,
        stdout: process.stdout,
        programArguments: [pin.path, "/old/openclaw.mjs", "gateway"],
      };
      await service.stage({
        ...args,
        runtimePinUpdate: { expected: readDaemonRuntimePin(scope, null), pin },
      ...
      const expected = readDaemonRuntimePinForInstall(scope, original, false);
      vi.mocked(readLaunchAgentProgramArguments).mockResolvedValueOnce(null);
      formerDefinition.mockResolvedValue({
        plistPath: state.statePath("former.plist"),
        command: changed ? { programArguments: ["/other/node", "gateway"] } : original,
      });
      const result = service.install({
        ...args,
        programArguments: [pin.path, "/new/openclaw.mjs", "gateway"],
        runtimePinUpdate: { expected, pin },
      });
      if (changed) {
        await expect(result).rejects.toThrow("changed during runtime pin planning");
        expect(native.install).not.toHaveBeenCalled();
        expect(readDaemonRuntimePin(scope, original).pin).toEqual(pin);
      } else {
        await result;
        expect(native.install).toHaveBeenCalledOnce();
        expect(readDaemonRuntimePin(scope, native.command).pin).toEqual(pin);
