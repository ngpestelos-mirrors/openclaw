...
export function isOpenClawStateDatabaseDefinitelyAbsent(
  ...
  try {
    ...
    if (
      ...
      openClawStateDatabaseCache.getCachedOpenClawStateDatabase(pathname, { readOnly: true })?.db
        .isOpen
    ) {
      return false;
    }
...
export function readCurrentOpenClawStateDatabaseContentVersion(
  ...
  const pathname = resolveReadOnlyPath(options);
  return stateSnapshotReads.exit(() => readAdmittedStateContentVersion(pathname, options.env));
}
