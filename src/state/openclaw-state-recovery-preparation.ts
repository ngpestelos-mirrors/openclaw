...
async function copyRecoveryGenerationExclusive(source: string, target: string): Promise<void> {
...
export async function prepareOpenClawStateRecoveryCopy(params: {
  ...
  params.assertOwned();
  await copyRecoveryGenerationExclusive(params.candidatePath, params.targetPath);
