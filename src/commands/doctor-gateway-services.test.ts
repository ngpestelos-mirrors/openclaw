...
import {
  ...
  createGatewayInstallPlanFixture,
  createGatewayCommand,
} from "./doctor-gateway-services.fixtures.test-support.js";
...
describe("maybeRepairGatewayServiceConfig", () => {
  beforeEach(() => {
    ...
    vi.clearAllMocks();
    mocks.readGatewayServiceCommandForMutation
      .mockReset()
      .mockImplementation(async (_service, env, opts) => {
  ...
  it.each(["current", "relocated"] as const)(
    "preserves %s service ownership while repairing managed environment drift",
    async (kind) => {
      mockProcessPlatform(kind === "current" ? "linux" : "darwin");
      ...
      mocks.readCommand.mockResolvedValue(kind === "current" ? existingCommand : null);
      if (kind === "relocated") {
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it.each(["Could not find service", "No such process"])(
    ...
    async (stderr) => {
      ...
      expect(rename).toHaveBeenCalledWith(LEGACY_MAC_PLIST, expect.any(String));
      const destination = String(rename.mock.calls[0]![1]);
      expect(path.dirname(destination)).toBe(path.join("/Users/test", ".Trash"));
      expect(path.basename(destination)).toMatch(/^com\.openclaw\.gateway-\d+\.plist$/);
