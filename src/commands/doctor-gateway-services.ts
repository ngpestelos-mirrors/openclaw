...
import { readGatewayServiceCommandForMutation, resolveGatewayService } from "../daemon/service.js";
...
export async function maybeRepairGatewayServiceConfig(
  ...
  try {
    ({ command } = await readGatewayServiceCommandForMutation(service, process.env));
  } catch {
    command = null;
  }
  if (!command) {
