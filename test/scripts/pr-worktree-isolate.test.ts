...
`,
  ...
  return { moves, disableFault: () => writeFileSync(disabled, "disabled\n") };
}
...
`,
  ...
  it("refuses a live checkout holder without moving its cwd", async ({ signal }) => {
    const f = preparedFixture();
    const holder = spawn(
      process.execPath,
      [
    ...
    (phase) => {
      ...
      if (phase !== "before") {
        ...
        if (phase === "links") {
          ...
          expect(JSON.parse(f.git(f.canonical, "cat-file", "blob", intentRef)).phase).toBe(
            "intent",
          );
        } else {
      ...
      expect(readFileSync(observer.moves, "utf8")).toBe("move\n");
    },
  );
