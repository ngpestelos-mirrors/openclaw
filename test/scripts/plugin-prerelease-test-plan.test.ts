  ...
  it("wires the full plugin prerelease plan into its release workflow", () => {
    ...
    expect(manifestEnv).toMatchObject({
      ...
      OPENCLAW_CI_DOCS_CHANGED:
        "${{ github.event_name == 'workflow_dispatch' && steps.runner_profile.outputs.node_runner_backend != 'runson' && 'true' || steps.docs_scope.outputs.docs_changed }}",
      OPENCLAW_CI_DOCS_ONLY:
        "${{ github.event_name == 'workflow_dispatch' && steps.runner_profile.outputs.node_runner_backend != 'runson' && 'false' || steps.docs_scope.outputs.docs_only }}",
      ...
      OPENCLAW_CI_RUN_ANDROID:
        "${{ github.event_name == 'workflow_dispatch' && ((inputs.release_gate && steps.runner_profile.outputs.node_runner_backend != 'runson') || inputs.include_android) && 'true' || steps.changed_scope.outputs.run_android || 'false' }}",
      OPENCLAW_CI_RUN_CONTROL_UI_I18N:
        "${{ github.event_name == 'workflow_dispatch' && steps.runner_profile.outputs.node_runner_backend != 'runson' && 'true' || steps.changed_scope.outputs.run_control_ui_i18n || 'false' }}",
      ...
      OPENCLAW_CI_RUN_NODE:
        "${{ github.event_name == 'workflow_dispatch' && steps.runner_profile.outputs.node_runner_backend != 'runson' && 'true' || steps.changed_scope.outputs.run_node || 'false' }}",
      OPENCLAW_CI_RUN_NODE_FAST_CI_ROUTING:
        "${{ github.event_name == 'workflow_dispatch' && steps.runner_profile.outputs.node_runner_backend != 'runson' && 'false' || steps.changed_scope.outputs.run_node_fast_ci_routing || 'false' }}",
      OPENCLAW_CI_RUN_NODE_FAST_ONLY:
        "${{ github.event_name == 'workflow_dispatch' && steps.runner_profile.outputs.node_runner_backend != 'runson' && 'false' || steps.changed_scope.outputs.run_node_fast_only || 'false' }}",
      OPENCLAW_CI_RUN_NODE_FAST_PLUGIN_CONTRACTS:
        "${{ github.event_name == 'workflow_dispatch' && steps.runner_profile.outputs.node_runner_backend != 'runson' && 'false' || steps.changed_scope.outputs.run_node_fast_plugin_contracts || 'false' }}",
      OPENCLAW_CI_RUN_SKILLS_PYTHON:
        "${{ github.event_name == 'workflow_dispatch' && steps.runner_profile.outputs.node_runner_backend != 'runson' && 'true' || steps.changed_scope.outputs.run_skills_python || 'false' }}",
      OPENCLAW_CI_RUN_UI_TESTS:
        "${{ github.event_name == 'workflow_dispatch' && steps.runner_profile.outputs.node_runner_backend != 'runson' && 'true' || steps.changed_scope.outputs.run_ui_tests || 'false' }}",
      OPENCLAW_CI_RUN_WINDOWS:
        "${{ github.event_name == 'workflow_dispatch' && steps.runner_profile.outputs.node_runner_backend != 'runson' && 'true' || steps.changed_scope.outputs.run_windows || 'false' }}",
