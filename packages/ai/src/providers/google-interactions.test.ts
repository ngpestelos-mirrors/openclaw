...
describe("google-interactions provider", () => {
  ...
  it.each([
    { name: "full", invoke: streamGoogleInteractions },
    { name: "simple", invoke: streamSimpleGoogleInteractions },
  ])(
    "returns an error stream for missing credentials through the direct $name entrypoint",
    async ({ invoke }) => {
      ...
      const stream = invoke(makeInteractionsModel("google-interactions"), basicContext);
      const events = [];
      ...
      expect(result).toMatchObject({
        ...
        stopReason: "error",
        errorMessage: "No API key for provider: google-interactions",
      });
  ...
  it.each([
    {
      name: "reported function-call usage",
      input: 100,
      cached: 0,
      output: 25,
      thought: 0,
      ...
      expectedInput: 100,
      expectedOutput: 25,
      expectedCost: 0.00015,
    },
    {
      name: "cached and thinking usage",
      input: 100,
      cached: 40,
      output: 20,
      thought: 30,
      ...
      expectedInput: 60,
      expectedOutput: 50,
      expectedCost: 0.00017,
    },
    {
      name: "authoritative total with other internal usage",
      input: 100,
      cached: 40,
      output: 20,
      thought: 30,
  ...
  ])("bills aggregate input once for $name", async (usage) => {
    vi.stubGlobal(
      "fetch",
      vi.fn(
        async () =>
          new Response(
            completedSse({
              usage: {
                ...
                total_thought_tokens: usage.thought,
                total_tool_use_tokens: usage.tool,
    ...
    const result = await streamGoogleInteractions(
      ...
      basicContext,
      { apiKey: "synthetic-usage-key" },
    ).result();
    expect(result.stopReason).toBe("stop");
    expect(result.usage).toMatchObject({
      input: usage.expectedInput,
      output: usage.expectedOutput,
      cacheRead: usage.cached,
      totalTokens: usage.expectedTotal,
      cost: {
        input: usage.expectedInput / 1_000_000,
        output: (usage.expectedOutput * 2) / 1_000_000,
        cacheRead: (usage.cached * 0.25) / 1_000_000,
        cacheWrite: 0,
      },
    });
    expect(result.usage.cost.total).toBeCloseTo(usage.expectedCost, 12);
  });
