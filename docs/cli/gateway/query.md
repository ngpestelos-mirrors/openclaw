...
<ParamField path="--timeout <ms>" type="number" default="10000">
  Probe timeout. Without an explicit value, the RPC probe uses 10 seconds and Windows PowerShell service inspection keeps its 60-second cold-start allowance. Explicit values also apply to native service probes. Each operation has its own budget; this is not an overall command deadline.
</ParamField>
