...
<AccordionGroup>
  <Accordion title="Forum topics and thread behavior">
    ...
    Topic entries inherit group settings unless overridden (`allowFrom`, `skills`, `systemPrompt`, `enabled`, `groupPolicy`). Every group/topic request requires a native bot mention or reply. Unread text is retained separately for each native conversation and agent, including across restarts. `agentId` is topic-only and does not inherit from group defaults. `topics."*"` sets defaults for every topic in that group; exact topic IDs still win over `"*"`.
