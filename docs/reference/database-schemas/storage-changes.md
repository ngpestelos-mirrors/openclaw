...
expiry, prefix lookup, and verdict admission. Registration binds each local approval
to one physical persistence owner through settlement and consumption. Later reads,
retries, and reconciliation reuse that owner even if ambient storage options change;
unproven terminal observations are reread there before settling its waiter.
An unknown verdict outcome gets one readback under the original physical database
admission and captured maintenance,
...
provenance. Such a verdict stays locally uncertain until its original deadline,
then rejects its local wait and handoffs without rewriting the durable winner or
authorizing timeout/ask-fallback replay. Other waiters remain active. A distinct
device or channel winner settles as an operator decision. A confirmed pending readback
