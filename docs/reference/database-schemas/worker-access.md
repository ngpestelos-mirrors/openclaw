...
Conversation-recall reset checks use `readSessionResetRecallCutoff` through the
private-local `memory-core-host-engine-sessions` SDK seam. It reads the existing
reset navigation projection in the same background reader pool, without hydrating
message bodies. Both paths
