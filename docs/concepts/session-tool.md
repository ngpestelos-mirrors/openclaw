...
- `thread: true` to bind an ACP spawn to a chat thread (Discord, Slack, etc.). Native sub-agents never bind a chat: they run in the background and report back to the requester. An older `thread: true` or `mode: "session"` sub-agent call still succeeds, runs unbound, and says so in its result note.
