...
The resulting managed worktree is attached to the session, and every agent run in that session uses its checkout. Multiple authorized sessions may attach to the same explicitly named checkout and run there concurrently; run leases track concurrent use without making either session the checkout's sole owner. The original `ownerId` remains creator metadata for compatibility, while durable session bindings govern access, cleanup, publication, and worker placement. Shared memberships require shared-state schema v18; after migration, older v17 runtimes reject the database instead of applying their single-owner cleanup rules. When the workspace is a repository subdirectory, the worktree is anchored at the repository root and the session runs from the matching subdirectory inside it. Session worktree creation uses the method's `operator.write` scope. Attaching to an existing worktree additionally requires mutation/participation access to every attached session, including archived sessions whose files remain in the checkout, preventing another user from exposing uncommitted files or secrets by guessing a name. Repository checkout/ref hooks and filesystem monitors are always disabled. The `.openclaw/worktree-setup.sh` step runs only for an `operator.admin` caller; retries evaluate the current caller's scope rather than retaining the original caller's permission. `.worktreeinclude` provisioning still applies to every caller. Deleting a session removes its binding and snapshots/removes the checkout only after the final active session detaches. Hourly cleanup considers every active attached session and does not retire a shared checkout merely because its creator disappeared or became inactive. Removed worktrees remain restorable from their snapshots as described below.

Archiving a session commits its archive state, then deactivates that session's attachment. The live checkout remains while another active session is attached. The final archive snapshots and removes it while retaining inactive bindings so any attached archived session can restore the same checkout later. Restore is serialized with allocation/removal, revalidates the session lifecycle, and reactivates only the restoring session; it never overwrites a live shared checkout. A failed metadata write leaves the checkout and active binding untouched. If cleanup cannot finish safely, the session remains archived, its files stay preserved, and the error explains that cleanup is pending. Repeating archive retries cleanup; startup and hourly garbage collection also retry any checkout that remains. Automatic session archival uses the same metadata-first ordering.
