...
Core-test-only PRs use targeted type checks only when every selected test exists in the checkout. Shared source or mixed source/test changes keep the full type-check plan, including the existing core stripes on GitHub and hybrid profiles, even when local changed checks can select their consumers. Deleting a core test also retains that full plan.
