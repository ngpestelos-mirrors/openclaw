...
For an immediate main-tier CI run, choose **Main CI Hourly → Run workflow →
main**, or run:
...
children and security-only pushes do not. With `OPENCLAW_CI_ON_PUSH` unset,
...
If main-tier CI takes longer than an hour, the current run finishes while GitHub
