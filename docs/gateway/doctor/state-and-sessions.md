...
<AccordionGroup>
  <Accordion title="3. Legacy state migrations (disk layout)">
    ...
    With `OPENCLAW_FS_SAFE_NATIVE_MODE=off`, device identity and shared file-claim migrations use guarded same-directory hardlinks. Hardlink support and directory synchronization remain required; run `openclaw doctor --fix` against the same state/config before restarting the Gateway.
