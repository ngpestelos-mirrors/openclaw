...
<AccordionGroup>
  <Accordion title="3. Legacy state migrations (disk layout)">
    ...
    An interrupted partial copy is not a legacy claim and cannot block retry: the original or an existing import receipt remains authoritative. Changed sources, competing claims, uncertain ownership, and unrelated permission or synchronization failures remain fail-closed. Required directory synchronization is not bypassed. Session-history recovery archives have a separate exact-original-identity contract; this metadata fallback does not silently copy them. Keep native mode disabled if the platform cannot run the helper, then rerun `openclaw doctor --fix` against the same state/config.
