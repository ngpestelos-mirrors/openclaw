...
<AccordionGroup>
  <Accordion title="3. Legacy state migrations (disk layout)">
    ...
    Device identity and other shared metadata claims also work with `OPENCLAW_FS_SAFE_NATIVE_MODE=off`. They first try guarded, same-directory hardlink publication without overwriting an existing claim. If native mode is explicitly off and the link operation itself fails with `EACCES` or `EPERM`, Doctor stages a private copy instead. It verifies and syncs the temporary copy before renaming it inside its exclusively owned staging directory; the original stays in place through the verified SQLite decision. Cleanup rechecks the original and the separate copy before removing either.

    An interrupted partial copy is not an active import claim: the original or an existing import receipt remains authoritative. If synchronization fails after the original identity file is removed, the next Doctor run finds the private staged copy, checks its receipt and canonical SQLite key, removes it, and reports the cleanup. Unverified recovery material stays in place with a diagnostic; it does not replace a valid canonical identity. Changed sources, competing claims, uncertain ownership, and unrelated permission or synchronization failures remain fail-closed. Required directory synchronization is not bypassed. Keep native mode disabled if the platform cannot run the helper, then rerun `openclaw doctor --fix` against the same state/config.

    Coordinator locks keep their existing `/tmp` location where that directory exists. On systems without `/tmp`, including stock Termux, they use the platform temporary directory instead.
    ...
    If native mode is off and the filesystem denies the first session-archive hardlink with `EACCES` or `EPERM`, Doctor can retain the verified original session files instead of archiving them. SQLite remains authoritative: retained originals neither block startup nor replay over current session edits or deletions. Doctor reports this archival limitation once when recording the verified import. Later runs can retry archival without importing the files again. Already-published archives keep their existing recovery rules; failed, changed, or unverified imports do not qualify.
