...
function sizingMenu(panel: DesktopPanelElement): HTMLSelectElement {
...
function selectSizing(panel: DesktopPanelElement, mode: string): void {
...
describe("embedded desktop panel presentation", () => {
  ...
  it.each(
    [false, true].flatMap((documentMode) =>
      ...
      ].map((permission) => ({ documentMode, ...permission })),
    ),
  )(
