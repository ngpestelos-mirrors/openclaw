...
const suite = createControlUiE2eSuite({
  ...
  // This suite serves the existing production build through its own real Gateway.
  startServer: async () => ({ baseUrl: "http://127.0.0.1/", close: async () => {} }),
});
    ...
    async (context) => {
      ...
      await suite.runScenario(context, {
        ...
        run: async () => {
          ...
          const colors = await observerCanvas.evaluate((element) => {
            const canvas = element as HTMLCanvasElement;
            const pixels = canvas
              .getContext("2d")!
          ...
          const inputAttempt = await page.evaluate(() => {
            ...
            const keys = [..."printf observer > /tmp/openclaw-desktop-resize-input"].map((key) =>
              key.codePointAt(0)!,
            );
            keys.push(0xff0d);
            ...
            JSON.stringify(
              {
                ...
                samples,
                pixels: { distinctSampledColors: colors },
              },
              null,
              2,
            ),
